...
export const DocumentsPage: FC = () => {
    ...
    const [fileToDelete, setFileToDelete] = useState<UploadedDocumentFile | null>(null);
    ...
    const openDeleteModal = (file: UploadedDocumentFile, e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    const loadFiles = () => {
        ...
        if (saved) {
            try {
                const parsed: UploadedDocumentFile[] = JSON.parse(saved);
            ...
            <Page header sidebar>
                <div className={style.audio_list}>
                    <div className={style.audio_list_title}>
                        <p className={style.audio_list__label}>
                            Все Документы
                        </p>
                        <NavButton className={style.add_audio__button} icon={<AddIcon className={style.add_audio__icon}/>} label='Загрузить файлы' path='/documents/upload'/>
                    </div>
                            ...
                            <table className={style.table}>
                                ...
                                <tbody>
                                    ...
                                    <tr
                                        ...
                                        key={file.id}
                                        onClick={() => {
                                        if (file.status !== 'loading') navigate(`/documents/${file.id}`);
                                        }}
                                    >
